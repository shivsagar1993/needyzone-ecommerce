...
async function updateCustomerOrder(request, response) {
  try {
    ...
    // Merge existing order with incoming fields for flexible partial or full updates
    const mergedData = {
      name: request.body.name !== undefined ? request.body.name : existingOrder.name,
      lastname: request.body.lastname !== undefined ? request.body.lastname : existingOrder.lastname,
      phone: request.body.phone !== undefined ? request.body.phone : existingOrder.phone,
      email: request.body.email !== undefined ? request.body.email : existingOrder.email,
      company: request.body.company !== undefined ? request.body.company : (existingOrder.company || "Individual"),
      adress: request.body.adress !== undefined ? request.body.adress : existingOrder.adress,
      apartment: request.body.apartment !== undefined ? request.body.apartment : (existingOrder.apartment || "1"),
      postalCode: request.body.postalCode !== undefined ? request.body.postalCode : existingOrder.postalCode,
      status: request.body.status !== undefined ? request.body.status : existingOrder.status,
      city: request.body.city !== undefined ? request.body.city : existingOrder.city,
      country: request.body.country !== undefined ? request.body.country : existingOrder.country,
      orderNotice: request.body.orderNotice !== undefined ? request.body.orderNotice : (existingOrder.orderNotice || ""),
      total: request.body.total !== undefined ? request.body.total : existingOrder.total,
    };

    // Server-side validation for merged update data
    const validation = validateOrderData(mergedData);
...
async function deleteCustomerOrder(request, response) {
  try {
    ...
    // Atomically delete linked order products and the order itself
