...
const createProduct = asyncHandler(async (request, response) => {
  ...
  if (!title || !title.trim()) {
    throw new AppError("Product Title is required. Please provide a descriptive title.", 400);
  }

  if (!merchantId) {
    throw new AppError("Merchant / Vendor is required. Please select a merchant.", 400);
  }

  if (!categoryId) {
    throw new AppError("Category Department is required. Please select a category.", 400);
  }

  if (!slug || !slug.trim()) {
    throw new AppError("URL Slug is required. Please enter or generate a slug.", 400);
  }

  const numericPrice = parseInt(price, 10);
  if (price === undefined || isNaN(numericPrice) || numericPrice <= 0) {
    throw new AppError("Price must be a valid positive number greater than 0.", 400);
  }

  const numericInStock = inStock !== undefined ? parseInt(inStock, 10) : 1;
  ...
  const product = await prisma.product.create({
    data: {
      ...
      slug: slug.trim(),
      title: title.trim(),
      mainImage: (mainImage && mainImage.trim()) ? mainImage.trim() : "product_placeholder.jpg",
      price: numericPrice,
      rating: 5,
      description: description ? description.trim() : "",
      manufacturer: (manufacturer && manufacturer.trim()) ? manufacturer.trim() : "Generic",
      categoryId,
      inStock: isNaN(numericInStock) ? 1 : numericInStock,
    },
  });
...
const updateProduct = asyncHandler(async (request, response) => {
  ...
  if (!existingProduct) {
    throw new AppError("Product not found. It may have been removed.", 404);
  }

  const numericPrice = price !== undefined ? parseInt(price, 10) : existingProduct.price;
  const numericInStock = inStock !== undefined ? parseInt(inStock, 10) : existingProduct.inStock;
  const numericRating = rating !== undefined ? parseInt(rating, 10) : existingProduct.rating;
  ...
  const updatedProduct = await prisma.product.update({
    ...
    data: {
      ...
      price: isNaN(numericPrice) ? existingProduct.price : numericPrice,
      rating: isNaN(numericRating) ? existingProduct.rating : numericRating,
      description: description !== undefined ? description : existingProduct.description,
      manufacturer: manufacturer !== undefined ? manufacturer : existingProduct.manufacturer,
      categoryId: categoryId || existingProduct.categoryId,
      inStock: isNaN(numericInStock) ? existingProduct.inStock : numericInStock,
    },
  });
...
const deleteProduct = asyncHandler(async (request, response) => {
  ...
  // Atomically clean up related records and delete the product
  await prisma.$transaction(async (tx) => {
    // 1. Remove references from customer_order_product table
    ...
    // 2. Remove associated gallery images
    ...
    // 4. Disassociate any bulk upload items
    await tx.bulk_upload_item.updateMany({
      where: {
        productId: id,
      },
      data: {
